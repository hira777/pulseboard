...
export function render404Html(options?: { title?: string; message?: string }) {
  const title = options?.title ?? '404 – Not Found'
  const message = options?.message ?? 'お探しのページは見つかりませんでした。'
