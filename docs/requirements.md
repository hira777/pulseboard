...
- **SKU（Stock Keeping Unit）**:
  機材の種類を表す管理単位。予約時は SKU ごとに数量を確保する。
  例: "Camera A" という SKU に対して「2 台」を予約。

- **個体**:
  SKU の実物。シリアル番号や状態（`available`, `repair`, `lost` など）を持つ。
  引渡時に予約済みの SKU 数量に対して実際の個体が割り当てられる。
...
- 仮押さえ (Hold): TTL（Time to Live）=10 分。超過で自動解放
...
- すべての SKU で個体管理を有効化できる
- 予約時: SKU レベルで数量確保
- 引渡時: 個体を割り当て
- キット: セット定義が可能。単品追加も可
- 返却時の不具合は個体ステータスを変更し可用枠から除外
...
- **正常系**:
  ...
  2. 日時・部屋・機材・スタッフを指定
  3. 仮押さえ（10 分）が作成される
  4. 確定操作を行うと、競合・例外日チェックが実施される
  5. 問題がなければ `confirmed` 状態で保存
- **例外系**:
  - 可用枠が存在しない → 候補なしを返す
  - 競合検知 → エラー（409）
  - TTL 超過 → 仮押さえ無効、再入力が必要
...
- **権限**: admin, member
- **操作**:
  - 予約の状態遷移（`pending` → `confirmed` → `in_use` → `completed` / `canceled`）
  - 内部ノートの追加・編集
  - メッセージ投稿（スレッドチャット）
  - 機材割当・解除
  - 監査ログの参照
...
| 仮押さえ TTL | 10 分 | ウィザード入力完了までの想定時間 | ○（システム設定） |
| キャンセル規定 | 24 時間前まで無料 | スタジオ標準規約を想定 | ○（テナント設定） |
| 予約状態の初期値 | `pending` | 作成直後は仮状態 | × |
...
- 予約を **作成**できる（仮押さえ → 確定、競合/例外日チェック含む）
