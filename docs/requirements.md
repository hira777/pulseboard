...
本プロダクトの中核機能。撮影スタジオの「部屋」と「機材（個体管理あり）」を同時に扱う実務運用を想定する。すべてのリソースはテナントIDで強制的に分離し、異なるテナントのデータを参照できないことを前提とする。
...
- テナント整合性: 予約・メッセージ・機材割当などの関連テーブルでテナントIDを複合外部キーに含め、DBレベルで他テナント参照を禁止する。
