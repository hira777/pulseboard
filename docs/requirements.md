...
本プロダクトの中核機能。撮影スタジオの「部屋」と「機材（個体管理あり）」を同時に扱う実務運用を想定する。すべてのリソースはテナント ID で強制的に分離し、異なるテナントのデータを参照できないことを前提とする。
...
- 入力必須: 部屋（`room_id`）と開始/終了時刻（`start_at` / `end_at`）。その他（サービス・顧客・スタッフ・機材）は現時点では任意入力。
- 時間刻み: 既定 15 分（将来 5/10/30 分へ切替可能）
- タイムゾーン: 店舗 TZ 固定（例: Asia/Tokyo）。DB には UTC で保存し、アプリ層で店舗 TZ ↔ UTC を変換（DST は考慮しない）。表示・入力は店舗 TZ に統一。
...
- テナント整合性: 予約・メッセージ・機材割当などの関連テーブルでテナント ID を複合外部キーに含め、DB レベルで他テナント参照を禁止する。
...
- 営業時間: 曜日ごとに店舗/部屋単位で保持。`rooms.open_hours` は `{ "mon": [{"start":"09:00","end":"18:00"}], ... }` のように曜日キーごとに複数枠を持つ JSON。`end` が `start` より小さい枠は翌日に跨ぐ営業（例: `23:00-02:00`）として解釈する。24 時間営業は `00:00-24:00` の 1 枠で表現し、判定ロジックを統一する。
- 例外日: 店舗/部屋/機材/スタッフ単位で設定可能（祝日や臨時変更は `calendar_exceptions` で上書きし、`open_hours` は定常パターンに限定）。`calendar_exceptions.target_id` を `NULL` にすると、その scope カテゴリ全体（例: scope=`room` なら全室）を休業扱いにできる。
- 判定は「予約が占有する全リソース」に対して適用

### 5.3 機材の管理方式

- すべての SKU で個体管理を有効化できる（`track_serial=true` の場合は個体登録が必須）。
- 予約確定時: システムが空いている個体を自動で選び、必要数量ぶんの個体 ID を `reservation_equipment_items` に保存。
...
- 予約可能枠検索: テナントのタイムゾーンでリクエスト日時を曜日・時刻に分解し、対応する `open_hours` 枠へ `start <= 時刻 < end` でマッチするか判定。その後、例外日と重複（部屋・機材・スタッフ）を順にチェックする。候補が多すぎるとレスポンスが重くなるため、返却件数は最大 50 件程度に制限することを推奨する（n ≤ 50）。
...
- **正常系**:
  1. ユーザーがサービスを選択
  2. 日時（開始/終了）と部屋を必須で指定し、必要に応じてサービス・機材・スタッフを追加
...
- **予約可能枠検索 API**: 最大 n=50 件を返却対象、P95 < 800ms を目標
...
- **性能試験**: 予約作成・予約可能枠検索でピーク負荷を想定したテストを実施
...
| パフォーマンス(API) | 予約可能枠検索 API | n ≤ 50 件、P95 < 800ms | 負荷試験・APM |
...
- **予約可能枠検索** が条件（期間/サービス/部屋/機材/スタッフ）で動作する（上限 50）

  - 出典: use-cases.md#uc-5-予約可能枠を検索する / features.md#予約可能枠検索
